...
color color_pixel(vec2 pixel_pos, vec2 resolution){
    ...
    auto t = 0; //ray_casting(ro,rd,world);
    if (t > 0){
        vec3 n = 0;//unit_vector(calcNormal(ro+rd*t,world));
        vec3 sun_dir = unit_vector(vec3(0.6, 0.35, 0.5));
        auto h = 0; //ray_casting(ro+rd*t+0.01*n,sun_dir,world);
        if (h > 0) c = color(0);
